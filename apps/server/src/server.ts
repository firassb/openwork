...
function createRoutes(
  ...
  addRoute(routes, "POST", "/auth/login", "none", async (ctx) => {
    const body = await ctx.request.json() as { email?: string; password?: string };
    const adminEmail = (process.env.OPENWORK_ADMIN_EMAIL ?? "admin@openwork.com").trim().toLowerCase();
    const adminPassword = (process.env.OPENWORK_ADMIN_PASSWORD ?? "OpenWorkAdmin2024!").trim();
    if (!body.email || !body.password) {
      throw new ApiError(400, "bad_request", "Email and password are required");
    }
    if (body.email.trim().toLowerCase() !== adminEmail || body.password !== adminPassword) {
      throw new ApiError(401, "unauthorized", "Invalid credentials");
    }
    return jsonResponse({ ok: true, token: config.token, email: adminEmail });
  });
