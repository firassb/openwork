...
import { ConfigView } from "@/react-app/domains/settings/pages/config-view";
...
function SettingsRouteContent(props: SettingsSurfaceProps = {}) {
  ...
  const connectedProviders = providers.flatMap((provider) =>
    providerConnectedIdSet.has(provider.id) && provider.id !== "opencode"
      ? [{
          id: provider.id,
      ...
      case "server":
        return (
          <ConfigView
            ...
            openworkServerHostInfo={openworkServerSnapshot.openworkServerHostInfo}
            runtimeWorkspaceId={runtimeWorkspaceId}
            updateOpenworkServerSettings={(next) => openworkServerStore.updateOpenworkServerSettings(next)}
            resetOpenworkServerSettings={() => openworkServerStore.resetOpenworkServerSettings()}
            testOpenworkServerConnection={(next) => openworkServerStore.testOpenworkServerConnection(next)}
            canReloadWorkspace={Boolean(openworkClient && (selectedWorkspace?.id || selectedWorkspaceId))}
            reloadWorkspaceEngine={reloadWorkspaceEngineFromUi}
